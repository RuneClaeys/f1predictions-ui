      ...
      {grandPrix.map((gp) => {
        return (
          <ListGroup.Item key={gp.id} as="li" className="d-flex justify-content-between align-items-start p-0">
            <NavLink to={!upcomming ? "/result/" + gp.id : gp.diff <= 1 ? `/prediction/` + gp.id : ""} className="d-flex align-items-center w-100 text-decoration-none text-black px-2">
...
const GrandPrixHistory = ({ overview, loading }) => {
  ...
  const historyGPs = useMemo(() => {
    return overview.grand_prix.filter((gp) => gp.qualifying_start_timestamp < new Date()).sort((a, b) => a.qualifying_start_timestamp - b.qualifying_start_timestamp)
  }, [overview])
